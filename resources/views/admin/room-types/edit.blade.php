...
@section('content')
    ...
    <h1 class="text-3xl font-semibold mb-6 text-gray-900">Editar tipo de habitación</h1>
    ...
    <form action="{{ route('admin.room-types.update', $roomType) }}" method="POST" enctype="multipart/form-data" 
        ...
        <div class="mb-4">
            ...
            <textarea id="description" name="description" rows="4" required
                ...
                class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $roomType->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="number" step="0.01" id="base_price" name="base_price" value="{{ old('base_price', $roomType->base_price) }}"
                    min="0" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
        </div>
        ...
        <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Imagen actual:</label>
            <div class="flex items-center space-x-4">
                <img src="{{ asset('storage/' . $roomType->main_image) }}" 
                     alt="{{ $roomType->name }}" 
                     class="h-20 w-20 rounded-lg object-cover border">
                <div class="text-sm text-gray-600">
                    <p>Si selecciona una nueva imagen, esta será reemplazada.</p>
                </div>
            </div>
        </div>
        @endif
        ...
        <!-- Nueva imagen -->
        <div class="mb-6">
            <label for="main_image" class="block mb-1 font-medium text-gray-700">
                {{ $roomType->main_image ? 'Nueva imagen:' : 'Imagen principal:' }}
            </label>
